        ...
        return angular.module("angularMoment", []).constant("angularMomentConfig", {
            ...
            return function(scope, element, attr) {
                ...
                function updateTime(momentInstance) {
                    var daysAgo = getNow().diff(momentInstance, "day"), showFullDate = fullDateThreshold && daysAgo >= fullDateThreshold;
                    if (showFullDate ? element.text(momentInstance.format(fullDateFormat)) : element.text(momentInstance.from(getNow(), withoutSuffix)), 
                    titleFormat && !element.attr("title") && element.attr("title", momentInstance.local().format(titleFormat)), 
                    !showFullDate) {
                ...
                scope.$watch(modelName, function(value) {
                    ...
                    currentFrom = "undefined" == typeof value || null === value || "" === value ? null : moment(value), 
                    updateMoment();
            ...
            this.preprocessors = {
                ...
                angularMomentConfig.timezone = timezone, $rootScope.$broadcast("amMoment:timezoneChanged");
                ...
                return (timezone = timezone || angularMomentConfig.timezone) ? (timezone.match(/^Z|[+-]\d\d:?\d\d$/i) ? aMoment = aMoment.utcOffset(timezone) : aMoment.tz ? aMoment = aMoment.tz(timezone) : $log.warn("angular-moment: named timezone specified but moment.tz() is undefined. Did you forget to include moment-timezone.js?"), 
                aMoment) : aMoment;
            };
            ...
            function amCalendarFilter(value, preprocess, timezone) {
                ...
                return date.isValid() ? amMoment.applyTimezone(date, timezone).calendar() : "";
            }
            ...
            function amDifferenceFilter(value, otherValue, unit, usePrecision, preprocessValue, preprocessOtherValue) {
                ...
                var date2;
                if ("undefined" == typeof otherValue || null === otherValue) date2 = moment(); else if (otherValue = amMoment.preprocessDate(otherValue, preprocessOtherValue), 
                date2 = moment(otherValue), !date2.isValid()) return "";
            ...
            function amDateFormatFilter(value, format, preprocess, timezone, inputFormat) {
                ...
                var date = moment(value);
                return date.isValid() ? amMoment.applyTimezone(date, timezone).format(format) : "";
            }
            ...
            function amTimeAgoFilter(value, preprocess, suffix, from) {
                var date, dateFrom;
                return "undefined" == typeof value || null === value ? "" : (value = amMoment.preprocessDate(value, preprocess), 
                date = moment(value), date.isValid() ? (dateFrom = moment(from), "undefined" != typeof from && dateFrom.isValid() ? amMoment.applyTimezone(date).from(dateFrom, suffix) : amMoment.applyTimezone(date).fromNow(suffix)) : "");
            }
            return amTimeAgoFilter.$stateful = angularMomentConfig.statefulFilters, amTimeAgoFilter;
        } ]).filter("amSubtract", [ "moment", "angularMomentConfig", function(moment, angularMomentConfig) {
            function amSubtractFilter(value, amount, type) {
                return "undefined" == typeof value || null === value ? "" : moment(value).subtract(parseInt(amount, 10), type);
            }
            return amSubtractFilter.$stateful = angularMomentConfig.statefulFilters, amSubtractFilter;
        } ]).filter("amAdd", [ "moment", "angularMomentConfig", function(moment, angularMomentConfig) {
            function amAddFilter(value, amount, type) {
                return "undefined" == typeof value || null === value ? "" : moment(value).add(parseInt(amount, 10), type);
            }
            return amAddFilter.$stateful = angularMomentConfig.statefulFilters, amAddFilter;
        } ]);
    }
    "function" == typeof define && define.amd ? define([ "angular", "moment" ], angularMoment) : "undefined" != typeof module && module && module.exports ? (angularMoment(angular, require("moment")), 
    module.exports = "angularMoment") : angularMoment(angular, ("undefined" != typeof global ? global : window).moment);
}();